        ...
        <div className="glass-panel p-6 rounded-2xl border-white/5 bg-stone-900/10 space-y-3">
            <div className="flex items-center justify-between border-b border-white/5 pb-2">
                ...
                <div className="flex items-center gap-2">
                    <select
                        aria-label="Créature à ajouter comme forme"
                    ...
                    <button onClick={addCustom} className="text-stone-400 hover:text-primary-400 text-sm" title="Forme personnalisée">+ Custom</button>
                </div>
            </div>
            {active && (
            ...
            {forms.length === 0 && <p className="text-[11px] text-stone-400 italic">Aucune forme.</p>}
            {forms.map((f, idx) => (
                <div key={idx} className={`rounded-lg border p-2 space-y-1.5 ${f.active ? 'border-purple-500/40 bg-purple-900/10' : 'border-white/5 bg-stone-950/20'}`}>
                    <div className="flex items-center gap-1.5 text-xs">
                        <button
                            ...
                            className={`text-[11px] uppercase font-black px-2 py-1 rounded border transition-all ${f.active ? 'bg-purple-500/20 border-purple-500 text-purple-200' : 'bg-stone-950 border-stone-700 text-stone-400 hover:text-white'}`}
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[11px]">
                        <span className="flex items-center gap-1">
                            ...
                            <button onClick={() => setHp(idx, f, -1)} className="text-stone-400 hover:text-red-400 w-4 text-center">−</button>
                            ...
                            <button onClick={() => setHp(idx, f, 1)} className="text-stone-400 hover:text-green-400 w-4 text-center">+</button>
                        </span>
