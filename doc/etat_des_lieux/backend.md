...
- **Suppressions, mesurées et non supposées** (`tests/Admin/BackOfficeSecurityTest.php::testAdminDeletesUserData`) : les 10 sections en consultation/suppression se suppriment toutes, **à condition de supprimer les feuilles avant les racines** — `Campaign` porte `orphanRemoval: true` sur ses quêtes, indices, séances, rencontres et adhésions, et `Character` la même chose sur ses voies de personnage ; Doctrine efface donc ces lignes en cascade dès que la racine disparaît. Purger une campagne à laquelle un **personnage** est réellement rattaché — l'état normal du produit, pas un cas de laboratoire — était **refusé (409)** : `character.campaign_id` (migration `Version20260301194224`) avait été posé sans clause `ON DELETE`, donc `RESTRICT` chez Postgres. **Corrigé le 2026-09-25 (`Version20260925090000`) : `ON DELETE SET NULL`.** Une fiche appartient à son joueur (`Character::$owner`), la campagne du MJ n'est qu'un rattachement — supprimer la campagne détache la fiche, sans jamais la supprimer avec elle. Décision de **produit** (dans les deux chemins : `DELETE /api/campaigns/{id}` par le MJ, et le back-office), pas un aménagement pour l'outil d'administration ; couverte par `CampaignSecurityTest::testDeletingCampaignDetachesPlayerCharactersInsteadOfBlockingOrDeletingThem` et `BackOfficeSecurityTest::testCampaignDeletionDetachesAttachedCharacters`.
...
- **Tests** : **143 tests / 1408 assertions**, mesurés répertoire par répertoire (la base `app_test` est partagée — deux exécutions concurrentes font échouer les deux ; **suite complète en ~2 min 30** depuis le 2026-09-25, contre plus de 25 min : `ApiSecurityTestCase` ne recrée plus le schéma avant chaque test mais vide les tables par `DELETE` en une transaction — `TRUNCATE` seul coûtait 6,1 s pour 32 tables sur ce stockage Docker/WSL2, contre ~40 ms) :
...
- **Intégration continue** : la suite complète (en ~2-3 min localement) s'exécute sur chaque PR et poussée vers master — aucune régression n'atteint l'historique.
