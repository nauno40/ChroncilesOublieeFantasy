...
const formate = (v: unknown): string => {
...
const classesCellule = (c: ColonneCompendium): string => [
    ...
    c.mono ? 'font-mono text-stone-300' : c.discret ? 'text-stone-400 text-xs' : 'text-stone-300',
...
export function CompendiumTable<T>({
    ...
    const minWidth = 220 + colonnes.length * 130 + (colonneFin ? 160 : 0);
                            ...
                            >
                                ...
                                {colonnes.map(c => (
                                    <td key={c.key} className={classesCellule(c)}>
                                        {c.wrap
                                            ? <span className="line-clamp-2 leading-snug">{formate(lire(ligne, c.key))}</span>
                                            : formate(lire(ligne, c.key))}
                                    </td>
                                ))}
                                {colonneFin && (
                ...
                {lignes.map(ligne => (
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-3">
                            {colonnes.map(c => {
                                const v = formate(lire(ligne, c.key));
                                if (v === '—') return null;
                                return (
