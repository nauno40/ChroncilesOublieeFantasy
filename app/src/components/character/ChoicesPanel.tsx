...
export const ChoicesPanel: React.FC<Props> = ({ character, setCharacter, races, profiles, allVoies }) => {
    ...
    return (
        <div className="glass-panel p-6 rounded-2xl border-white/5 bg-stone-900/10 space-y-3">
            <h3 className="text-stone-400 font-display font-bold uppercase text-[11px] tracking-[0.2em] border-b border-white/5 pb-2">Capacités à choix</h3>
            {rows.length === 0 && <p className="text-[11px] text-stone-400 italic">Aucune capacité à choix.</p>}
            {rows.map(row => (
                <div key={`${row.idx}-${row.rank}`} className="space-y-1">
                    <label className="text-[11px] text-stone-300 font-bold">
                        {row.capName}
                        <span className="text-stone-400 font-normal"> — {row.voieName}</span>
                    </label>
                    {row.help && <p className="text-[11px] text-stone-400 italic leading-snug">{row.help}</p>}
