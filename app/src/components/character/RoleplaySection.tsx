...
const RollButton: React.FC<{ table: string[]; onRoll: (v: string) => void; title: string }> = ({ table, onRoll, title }) => (
    <button
        ...
        title={title}
        className="flex items-center gap-1 text-[11px] uppercase font-bold text-stone-400 hover:text-primary-400 border border-white/10 hover:border-primary-500/30 rounded px-2 py-1 transition-all active:scale-95"
    >
        <Dices size={12} /> Tirer
    </button>
);
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="glass-panel p-6 rounded-2xl border-white/5 bg-stone-900/10 space-y-3">
                ...
                <textarea
                    className="w-full bg-stone-950/40 border border-stone-800/50 rounded-xl p-4 text-stone-300 focus:border-primary-500/30 focus:bg-stone-900/40 outline-none h-32 resize-none transition-all font-body leading-relaxed placeholder:text-stone-400"
            ...
            <div className="glass-panel p-6 rounded-2xl border-white/5 bg-stone-900/10 space-y-3">
                <div className="flex items-center justify-between ml-1">
                    <label className="text-xs uppercase font-black text-red-400/80 tracking-[0.2em]">Travers / Défaut</label>
                    <RollButton table={TRAVERS} onRoll={v => setRp('flaw', v)} title="Tirer un travers (d20)" />
                </div>
                <textarea
                    className="w-full bg-stone-950/40 border border-stone-800/50 rounded-xl p-4 text-stone-300 focus:border-red-900/30 focus:bg-stone-900/40 outline-none h-32 resize-none transition-all font-body leading-relaxed placeholder:text-stone-400"
            ...
            <div className="glass-panel p-6 rounded-2xl border-white/5 bg-stone-900/10 space-y-3">
                ...
                <textarea
                    className="w-full bg-stone-950/40 border border-stone-800/50 rounded-xl p-4 text-stone-300 focus:border-purple-500/30 focus:bg-stone-900/40 outline-none h-32 resize-none transition-all font-body leading-relaxed placeholder:text-stone-400"
            ...
            <div className="glass-panel p-6 rounded-2xl border-white/5 bg-stone-900/10 space-y-3">
                <label className="text-xs uppercase font-black text-stone-400/60 tracking-[0.2em] ml-1">Notes</label>
                <textarea
                    className="w-full bg-stone-950/40 border border-stone-800/50 rounded-xl p-4 text-stone-300 focus:border-stone-500/30 focus:bg-stone-900/40 outline-none h-32 resize-none transition-all font-body leading-relaxed placeholder:text-stone-400"
